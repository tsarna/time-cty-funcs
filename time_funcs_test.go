...
func TestTimePart(t *testing.T) {
	...
	ts := NewTimeCapsule(time.Date(2024, 1, 15, 10, 30, 45, 123456789, time.UTC))
	tests := []struct {
		part string
		want int64
	}{
...
func TestTimePartISOWeek(t *testing.T) {
	...
	ts := NewTimeCapsule(time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC))
...
func TestTimePartISOYearBoundary(t *testing.T) {
	...
	ts := NewTimeCapsule(time.Date(2019, 12, 30, 0, 0, 0, 0, time.UTC))
	year, err := TimePartFunc.Call([]cty.Value{ts, cty.StringVal("isoyear")})
	require.NoError(t, err)
	y, _ := year.AsBigFloat().Int64()
